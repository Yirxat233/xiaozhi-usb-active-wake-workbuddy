...
定制固件源码位于 `firmware/xiaozhi-esp32`。在 ESP-IDF 5.5.2 终端运行 `powershell -File .\firmware\build-firmware.ps1` 可重新构建；GitHub Release 提供已验证硬件可直接烧录的 wb3 合并固件。
...
在小智后台复制 `wss://api.xiaozhi.me/mcp/?token=...` 接入点，通过进程环境变量启动。完整地址包含密钥，不要提交到仓库或写入日志：
...
Bridge 会作为 MCP Server 主动连接小智云端，完成 `initialize`、工具发现和 `tools/call`，断线后按指数退避自动重连。调试台只显示去掉查询参数后的地址，并展示连接状态、重试次数、收到的消息数和工具调用数。
...
- `auto`/`desktop` 模式取得 Session ID 后打开 `workbuddy:///task/<sessionId>`，桌面端与 Bridge 映射到同一份真实历史记录，不写 WorkBuddy 数据库或索引。
...
小智主动调用 WorkBuddy 的 MCP 路径与异步主动播报是两个方向的链路。现在可用 `UsbXiaozhiNotifier` 将 WorkBuddy 完成/提问事件送回本机 USB 连接的小智。设备忙碌时排队，播放完成后才记录成功。队列仍在内存中；长期部署仍需持久化播报队列和更多设备绑定能力。
