...
Es8311AudioCodec::Es8311AudioCodec(void* i2c_master_handle, i2c_port_t i2c_port, int input_sample_rate, int output_sample_rate,
    ...
    // 先做 I2C 寄存器配置，再开 I2S/MCLK，避免时钟起来后总线读写更不稳定
    ...
    // ES8311 首包 I2C 偶发失败（驱动注释也提到）；与 AXP 共总线时更明显，加重试
    for (int attempt = 1; attempt <= 5; attempt++) {
        codec_if_ = es8311_codec_new(&es8311_cfg);
        if (codec_if_ != nullptr) {
            ESP_LOGI(TAG, "Es8311AudioCodec initialized (attempt %d)", attempt);
            break;
        }
        ESP_LOGW(TAG, "Es8311 open failed, retry %d/5", attempt);
