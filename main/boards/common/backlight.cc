...
PwmBacklight::PwmBacklight(gpio_num_t pin, bool output_invert, uint32_t freq_hz, bool brightness_invert) : Backlight(), brightness_invert_(brightness_invert) {
...
void PwmBacklight::SetBrightnessImpl(uint8_t brightness) {
    // When brightness_invert_: hardware is "high PWM duty = dimmer", so invert so logical 100 = brightest
    if (brightness_invert_) {
