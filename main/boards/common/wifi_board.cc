...
void WifiBoard::StartWifiConfigMode() {
    ...
    // Prevent audio feedback/noise in WiFi config mode:
    // After the prompt sound plays, force-disable speaker output (mic input may still be used for acoustic provisioning).
    Application::GetInstance().Schedule([]() {
        ...
        if (codec && codec->output_enabled()) {
            ESP_LOGI(TAG, "Disabling speaker output to prevent audio feedback in WiFi config mode");
    ...
    // Also disable speaker output in config mode to avoid feedback/noise.
    Application::GetInstance().Schedule([]() {
        vTaskDelay(pdMS_TO_TICKS(3000));
        auto codec = Board::GetInstance().GetAudioCodec();
        if (codec && codec->output_enabled()) {
