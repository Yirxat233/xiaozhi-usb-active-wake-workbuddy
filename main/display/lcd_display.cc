...
void LcdDisplay::SetupUI() {
    ...
    /* Content - Chat area (fixed region right under emoji) */
    content_ = lv_obj_create(screen);
    lv_obj_set_style_radius(content_, 0, 0);
    lv_obj_set_width(content_, LV_HOR_RES);
    lv_obj_set_height(content_, LV_VER_RES / 3 * 2); // use middle/lower 2/3 for chat
    lv_obj_set_style_pad_all(content_, lvgl_theme->spacing(4), 0);
    lv_obj_set_style_border_width(content_, 0, 0);
    lv_obj_set_style_bg_opa(content_, LV_OPA_TRANSP, 0); // keep background transparent
    lv_obj_set_scrollbar_mode(content_, LV_SCROLLBAR_MODE_OFF);
    lv_obj_set_scroll_dir(content_, LV_DIR_VER);
    lv_obj_set_flex_flow(content_, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_flex_align(content_, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);
    lv_obj_set_style_pad_row(content_, lvgl_theme->spacing(4), 0);
    // Place chat region just below emoji
    lv_obj_align_to(content_, emoji_image_, LV_ALIGN_OUT_BOTTOM_MID, 0, lvgl_theme->spacing(4));
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    if (strcmp(role, "user") == 0) {
        ...
        // If this is the first visible message, place it just under the emoji and don't scroll
        if (child_count == 0 && emoji_image_ != nullptr) {
            lv_obj_align_to(container, emoji_image_, LV_ALIGN_OUT_BOTTOM_MID, 0, lvgl_theme->spacing(6));
        } else {
            // Auto-scroll to this container for later messages
        ...
        // If this is the first visible message, place it just under the emoji and don't scroll
        if (child_count == 0 && emoji_image_ != nullptr) {
            lv_obj_align_to(msg_bubble, emoji_image_, LV_ALIGN_OUT_BOTTOM_MID, 0, lvgl_theme->spacing(6));
        } else {
...
void LcdDisplay::SetupUI() {
    ...
    /* Place dialog text directly below emoji instead of at screen bottom */
    lv_obj_align_to(bottom_bar_, emoji_box_, LV_ALIGN_OUT_BOTTOM_MID, 0, lvgl_theme->spacing(4));
