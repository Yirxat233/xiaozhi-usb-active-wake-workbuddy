...
void LocalUsbBridge::Start() {
    usb_serial_jtag_driver_config_t config = { .tx_buffer_size = 4096, .rx_buffer_size = 8192 };
...
void LocalUsbBridge::Cancel() {
    ...
    if (mode == UsbBridgeMode::CloudInput) {
        ...
        if (app.GetDeviceState() == kDeviceStateSpeaking || app.GetDeviceState() == kDeviceStateConnecting)
...
void LocalUsbBridge::OnCloudReplyFinished() {
    if (!active_ || mode_ != UsbBridgeMode::CloudInput || !waiting_for_cloud_reply_) return;
...
void LocalUsbBridge::OnCloudRequestFailed(const char* error) {
    if (!active_ || mode_ != UsbBridgeMode::CloudInput) return;
    const auto id = completion_id_;
    active_ = false;
    mode_ = UsbBridgeMode::None;
    waiting_for_cloud_reply_ = false;
    completion_id_.clear();
    if (!id.empty()) Reply(id, "error", error);
}
...
void LocalUsbBridge::Run() {
    ...
    while (true) {
        ...
        if (count == 1) {
            if (ch == '\n') {
                if (!overflow && line.rfind("WB:", 0) == 0) Handle(line.substr(3));
                line.clear();
                overflow = false;
            } else if (ch != '\r') {
                if (line.size() < 4096) line.push_back(ch);
                else overflow = true;
            }
        } else {
...
void LocalUsbBridge::Handle(const std::string& line) {
    ...
    const auto id = get("id"), op = get("type"), session = get("session_id");
    if (id.empty() || id.size() > 80) { cJSON_Delete(root); return; }
    auto& app = Application::GetInstance();
    if (op == "status") {
        Reply(id, "status");
        ...
        if (session.empty() || session.size() > 80 || text.size() > 2400) {
            ...
            // All state transitions happen on the application task, like MQTT speak_request.
            app.Schedule([this, id, session, text, listen_after]() {
                ...
                // State-change processing resets the decoder before we acknowledge readiness.
                current.Schedule([this, id, session, text]() {
                    if (!active_ || session != session_ || Application::GetInstance().GetDeviceState() != kDeviceStateSpeaking) {
        ...
        if (session.empty() || session.size() > 80 || text.empty() || text.size() > 2400) {
            ...
            app.Schedule([this, id, session, text]() {
                ...
                current.BeginExternalAudioInput([this, id, session](bool ready) {
                    if (!ready || !active_ || mode_ != UsbBridgeMode::CloudInput || session != session_) {
                        active_ = false;
                        mode_ = UsbBridgeMode::None;
        ...
        const auto expected_mode = cloud ? UsbBridgeMode::CloudInput : UsbBridgeMode::LocalPlayback;
        const auto expected_state = cloud ? kDeviceStateListening : kDeviceStateSpeaking;
        if (!active_ || mode_ != expected_mode || session != session_ || app.GetDeviceState() != expected_state) {
            Reply(id, "error", "session_inactive");
        } else if (!cJSON_IsNumber(seq) || seq->valuedouble != sequence_ || encoded.empty() || encoded.size() > 1800) {
            Reply(id, "error", "invalid_packet");
        } else {
            std::vector<uint8_t> payload(1350);
            size_t decoded = 0;
            int result = mbedtls_base64_decode(payload.data(), payload.size(), &decoded,
            ...
            if (result != 0 || decoded == 0 || decoded > 1275) {
                ...
                auto packet = std::make_unique<AudioStreamPacket>(AudioStreamPacket{
                    .sample_rate = cloud ? 16000 : 24000, .frame_duration = 60, .timestamp = 0, .payload = std::move(payload)});
                const bool accepted = cloud ? app.SendExternalAudio(std::move(packet))
                                            : app.GetAudioService().PushPacketToDecodeQueue(std::move(packet), false);
                if (accepted) {
                    sequence_++;
                    last_packet_us_ = esp_timer_get_time();
                    Reply(id, cloud ? "input_ack" : "audio_ack");
                } else Reply(id, "busy", "audio_queue_full");
            }
        }
        ...
        if (!active_ || session != session_) Reply(id, "error", "session_inactive");
        else {
            app.GetAudioService().WaitForPlaybackQueueEmpty();
            vTaskDelay(pdMS_TO_TICKS(150));
            app.Schedule([this, id, session]() {
                if (!active_ || session != session_) { Reply(id, "error", "interrupted"); return; }
                active_ = false;
                mode_ = UsbBridgeMode::None;
                ...
                Application::GetInstance().Schedule([this, id, listen_after]() {
                    Reply(id, "speak_done");
                    if (listen_after) Application::GetInstance().ToggleChatState();
                });
            });
        }
    } else if (op == "cancel") {
        if (session != session_) Reply(id, "error", "session_mismatch");
        else app.Schedule([this, id]() { Cancel(); Reply(id, "cancelled"); });
    } else Reply(id, "error", "unknown_type");
    cJSON_Delete(root);
}
