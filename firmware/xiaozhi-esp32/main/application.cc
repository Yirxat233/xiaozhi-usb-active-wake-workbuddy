    ...
    while (true) {
        ...
        if (bits & MAIN_EVENT_SEND_AUDIO) {
            while (auto packet = audio_service_.PopPacketFromSendQueue()) {
                // During a USB-injected turn, only the host audio is sent upstream. Drop live
                // microphone frames so they cannot be mixed into the synthetic user message.
...
void Application::BeginExternalAudioInput(std::function<void(bool)>&& on_ready) {
    ...
    Schedule([this, on_ready = std::move(on_ready)]() mutable {
        ...
        // Manual-stop mode makes the USB input boundary deterministic. The host sends the
        // equivalent of releasing a push-to-talk button after its last Opus frame.
...
void Application::FinishExternalAudioInput() {
    if (!external_audio_input_.exchange(false) || !protocol_) return;
    protocol_->SendStopListening();
}
...
void Application::ContinueHandleSpeakRequest(const std::string& session_id) {
    ...
    // Allow the speaking-state handler to reset the decoder before the server
    // is told to send audio; otherwise the first incoming frames can be cleared.
    Schedule([this, session_id]() {
        if (!protocol_ || GetDeviceState() != kDeviceStateSpeaking) return;
